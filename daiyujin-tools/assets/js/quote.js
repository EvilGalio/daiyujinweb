document.addEventListener("DOMContentLoaded", () => {
    ...
    function validateContactDetails() {
        ...
        if (!contact.customer_name || !contact.customer_email) {
            renderError("Please enter your name and email address before calculating.");
            return null;
        }
        return contact;
    }
    ...
    function estimateDisclaimer(customerName) {
        ...
        return `${opening} reference estimate is generated by our AI-assisted quoting system, which is continuously learning from manufacturing data. At this stage, the price is for reference only. For accurate pricing, please click ${FORMAL_QUOTE_LABEL}.`;
    }
    function escapeRegExp(v) { return String(v).replace(/[.*+?^${}()|[\]\\]/g, "\\$&"); }
    function formalQuoteText(v) {
        const labelPattern = new RegExp(escapeRegExp(FORMAL_QUOTE_LABEL), "gi");
        return esc(v)
            .replace(/contact our engineers/gi, match => `<a href="${FORMAL_QUOTE_URL}" target="_blank" rel="noopener">${match}</a>`)
            .replace(labelPattern, match => `<a href="${FORMAL_QUOTE_URL}" target="_blank" rel="noopener">${match}</a>`);
    }
